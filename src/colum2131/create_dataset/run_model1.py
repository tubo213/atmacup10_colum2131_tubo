from base_model1 import Feature, generate_features
...
class AcquisitiondateBasefeature(Feature):
    def create_features(self):
        ...
        #self.train['year_acquisition_date'] = train_date.map(lambda x: x.year)
        #self.test['year_acquisition_date'] = test_date.map(lambda x: x.year)
        ...
        #self.train['day_acquisition_date'] = train_date.map(lambda x: x.day)
        #self.test['day_acquisition_date'] = test_date.map(lambda x: x.day)
...
if __name__ == '__main__':
    ...
    generate_features(class_list, True)
