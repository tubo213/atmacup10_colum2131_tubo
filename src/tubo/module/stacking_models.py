...
class SVR(Base_Model):
    def __init__(self,kernel,C):
        self.kernel = kernel
        self.C = C

    def fit(self,x_train,y_train,x_valid,y_valid):
        model = svm.SVR(kernel=self.kernel,gamma='auto',C=self.C)
...
class Extra(Base_Model):
  def __init__(self,depth,n_est):
        self.depth=depth
        self.n_est=n_est
  def fit(self,x_train,y_train,x_valid,y_valid):
    model = ExtraTreesRegressor(max_depth=self.depth,n_estimators=self.n_est,random_state=0,n_jobs=-1)
    model.fit(x_train,y_train)
    return model
